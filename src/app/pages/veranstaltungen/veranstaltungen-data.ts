...
export interface VeranstaltungItem {
  ...
  /**
   * Teilnahme-Modus aus dem Joomla-Custom-Field "teilnahme".
   * Pflegbar im Backoffice als Single-List mit den drei Optionen
   * Frei / Voranmeldung / Eintrittskarte.
   *
   * Fallback "frei" wenn das Feld leer ist oder einen unbekannten
   * Wert hat — wir wollen keinen Event komplett zurueckhalten nur
   * weil das Feld nicht gepflegt wurde.
   */
  readonly teilnahme: TeilnahmeMode;
  readonly bodyHtml: string;
  readonly excerpt: string;
}

export type TeilnahmeMode = 'frei' | 'voranmeldung' | 'eintrittskarte';
 ...
 */
/**
 * Mapped die Joomla-Single-List-Werte des Felds "teilnahme" auf das
 * interne Enum. Joomla nutzt im Backoffice die Labels "Frei",
 * "Voranmeldung", "Eintrittskarte" — sowohl als Key als auch als
 * Display. Sicherheitshalber lowercased + getrimmt, damit kleine
 * Tippvarianten ("frei", "Voranmeldung ") nicht direkt zum Fallback
 * fuehren.
 */
function mapTeilnahme(rawKey: string | undefined): TeilnahmeMode {
  const k = (rawKey ?? '').trim().toLowerCase();
  if (k === 'voranmeldung') return 'voranmeldung';
  if (k === 'eintrittskarte') return 'eintrittskarte';
  return 'frei';
}
...
export function mapVeranstaltung(j: JoomlaArticle): VeranstaltungItem {
  ...
  const teilnahmeRaw = readListField(attrs['teilnahme']);
  const teilnahme = mapTeilnahme(teilnahmeRaw?.key);
