...
describe('mapVeranstaltung', () => {
  ...
  it('teilnahme: mapped Joomla-Single-List auf Enum (Frei/Voranmeldung/Eintrittskarte)', () => {
    const frei = mapVeranstaltung(attrs({ 'teilnahme': { 'Frei': 'Frei' } }));
    expect(frei.teilnahme).toBe('frei');

    const vor = mapVeranstaltung(
      attrs({ 'teilnahme': { 'Voranmeldung': 'Voranmeldung' } }),
    );
    expect(vor.teilnahme).toBe('voranmeldung');

    const karte = mapVeranstaltung(
      attrs({ 'teilnahme': { 'Eintrittskarte': 'Eintrittskarte' } }),
    );
    expect(karte.teilnahme).toBe('eintrittskarte');
  });

  it('teilnahme: Fallback auf "frei" wenn Feld fehlt oder unbekannter Wert', () => {
    const fehlt = mapVeranstaltung(mkJoomlaArticle());
    expect(fehlt.teilnahme).toBe('frei');

    const fremd = mapVeranstaltung(attrs({ 'teilnahme': { 'Sonstwas': 'Sonstwas' } }));
    expect(fremd.teilnahme).toBe('frei');
  });

  it('teilnahme: case-insensitive + trimmed (toleriert kleine Tippvarianten)', () => {
    const a = mapVeranstaltung(
      attrs({ 'teilnahme': { ' voranmeldung ': ' voranmeldung ' } }),
    );
    expect(a.teilnahme).toBe('voranmeldung');
  });
