...
/** ISO-Datum (YYYY-MM-DD) relativ zu heute, fuer datumsabhaengige Filter-Tests. */
...
describe('CotasApiService', () => {
  ...
  it('loadCatalog: flacht verschachtelte dance_classes und filtert vergangene Nicht-Club-Kurse raus', () => {
      ...
      dance_classes: {
        'ZG-A': [
          [
            ...
            // Laengst beendet (Start + 8 Wochen weit in der Vergangenheit)
            // + non-club -> rausfliegen
            mkClass({
              id: '2',
              kurs_bez: 'Alt',
            ...
            // Gestern gestartet, laeuft noch 8 Wochen + non-club -> bleibt.
            // (Regression 08/2026: der alte Startdatum-Filter hat solche
            // gerade gestarteten Kurse faelschlich ausgeblendet.)
            mkClass({
              id: '5',
              kurs_bez: 'Gerade gestartet',
              kategorie_priority: '10',
              kurs_beginn: isoDaysFromToday(-1),
              einheiten: '8',
              kzclub: '0',
            }),
          ],
    ...
    // 1 + 3 + 4 + 5 bleiben uebrig (nur id=2 = laengst beendeter Nicht-Club-Kurs)
    const flat = cat?.['classesByGroup'].get('ZG-A') ?? [];
    expect(flat).toHaveLength(4);
    expect(flat.find((c: CotasDanceClass) => c.id === '2')).toBeUndefined();
    // Der gerade gestartete, noch laufende Kurs bleibt sichtbar:
    expect(flat.find((c: CotasDanceClass) => c.id === '5')).toBeDefined();
