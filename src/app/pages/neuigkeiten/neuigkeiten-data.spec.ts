...
describe('mapNewsArticle', () => {
  ...
  it('Excerpt dekodiert HTML-Entitaeten (nbsp, amp, Umlaute)', () => {
    const a = mapNewsArticle(
      mkJoomlaArticle({
        text: '<p>Holstenk&ouml;ste!&nbsp;Stand am B&uuml;hnen&amp;Foyer</p>',
      }),
    );
    expect(a.excerpt).not.toContain('&nbsp;');
    expect(a.excerpt).not.toContain('&amp;');
    expect(a.excerpt).not.toContain('&ouml;');
    expect(a.excerpt).toContain('Holstenköste');
    expect(a.excerpt).toContain('Bühnen&Foyer');
  });

  it('Excerpt-Metadesc dekodiert HTML-Entitaeten ebenfalls', () => {
    const j = mkJoomlaArticle({ text: '<p>Body</p>' });
    (j as Record<string, unknown>)['metadesc'] = 'Caf&eacute; &amp; Tanz';
    const a = mapNewsArticle(j);
    expect(a.excerpt).toBe('Café & Tanz');
  });
});
