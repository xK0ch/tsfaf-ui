...
function normalizeCatalog(resp: CotasDanceClassesResponse): DanceClassCatalog {
  ...
  for (const tg of targetGroups) {
    ...
    const flat = flattenClasses(raw).filter(c => !hasEndedNonClub(c));
...
/**
 * Filtert Kurse deren Laufzeit komplett in der Vergangenheit liegt. Der
 * von Cotas gelieferte `finished`-Flag ist dafuer unzuverlaessig: bei
 * 8-Wochen-Kursen wird er offenbar erst nach `kurs_beginn + einheiten*7d`
 * gesetzt, sodass laengst beendete Kurse noch als finished=0 kommen und
 * damit sichtbar bleiben (User-Beschwerde 07/2026: alte Mai-Kurse
 ...
 *
 * WICHTIG: Massgeblich ist das ENDdatum, nicht das Startdatum. Ein frueher
 * Vergleich `kurs_beginn < heute` hat die gerade erst gestarteten Kurse
 * der neuen Saison ausgeblendet (Beschwerde 08/2026: die am 16./17.08.
 * begonnenen Herbstkurse verschwanden ab dem 18.08., obwohl sie noch
 * 8 Wochen laufen). Das Ende schaetzen wir als
 * `kurs_beginn + einheiten * 7 Tage` (ein Termin pro Woche) plus einen
 * Puffer-Tag, damit der Kurs am letzten Termin noch sichtbar ist.
 *
 * Fehlt `einheiten` oder ist 0 (z.B. Clubs, oder unsauber gepflegte
 * Daten), blenden wir NICHT aus — im Zweifel lieber zeigen als einen
 * aktuellen Kurs verstecken.
 *
 ...
 */
function hasEndedNonClub(c: CotasDanceClass): boolean {
  const isClub =
    c.kzclub === '1' ||
    (typeof c.kzclub === 'number' && c.kzclub === 1);
  if (isClub) return false;

  const beginn = parseIsoDate((c.kurs_beginn ?? '').trim());
  if (!beginn) return false;

  const units = parseInt(c.einheiten, 10);
  if (!Number.isFinite(units) || units <= 0) return false;

  // Geschaetztes Ende: ein Termin pro Woche, +1 Tag Puffer damit der Kurs
  // am Tag des letzten Termins noch angezeigt wird.
  const end = new Date(beginn.getTime());
  end.setDate(end.getDate() + units * 7 + 1);
...
/**
 * Parst "YYYY-MM-DD" zu einem lokalen Date (Mitternacht). Gibt null
 * zurueck wenn das Format nicht stimmt. Bewusst ohne `new Date(string)`,
 * weil das je nach Browser als UTC interpretiert wird und einen
 * Tagesversatz erzeugen kann.
 */
