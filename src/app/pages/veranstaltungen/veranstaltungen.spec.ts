...
function makeRoute(initialEventSlug = '') {
  const params$ = new BehaviorSubject(
    convertToParamMap(initialEventSlug ? { event: initialEventSlug } : {}),
  );
  const route = {
    queryParamMap: params$.asObservable(),
    setEvent(slug: string | null): void {
      params$.next(convertToParamMap(slug ? { event: slug } : {}));
    },
  };
  return route;
}
...
describe('Veranstaltungen', () => {
  function setup(opts: {
    ...
    const route = makeRoute(opts.initialEventSlug ?? '');
