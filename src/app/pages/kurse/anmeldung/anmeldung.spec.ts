...
describe('Anmeldung', () => {
  ...
  it('debit_charge=true erzwingt iban/bic/inhaber', () => {
    const fixture = setup();
    ...
    cmp.form.patchValue({ debit_charge: true });
    fixture.detectChanges();
  ...
  it('submit baut korrekten Payload: telefon Sentinel "-" wenn leer, Geburtstag in DE-Format', () => {
    ...
    fillRequiredFields(cmp.form as unknown as Record<string, unknown>);
    cmp.form.patchValue({ telefon: '' }); // optional, leer
    cmp.submit();
    ...
    expect(submitCapture.payload!.debit_charge).toBe(0);
  });
