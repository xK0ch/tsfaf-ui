...
/**
 * HTML-Entitaeten (&nbsp;, &amp;, &auml;, &#39; etc.) in Klartext
 * aufloesen. Notwendig fuer Excerpts die mit {{ }}-Interpolation
 * gerendert werden — Angular escapt da nicht, also wuerde "&nbsp;"
 * wortwoertlich sichtbar bleiben.
 *
 * Browser-Trick mit textarea: setzt innerHTML (dekodiert) und liest
 * dann den Plain-Text wieder zurueck. Falls kein DOM verfuegbar
 * (theoretisch SSR), Fallback fuer die haeufigsten Entitaeten.
 */
 ...
 * Im zweiten Pfad werden HTML-Entitaeten aufgeloest, damit Sachen wie
 * "&nbsp;" oder "Bühne&amp;Foyer" nicht wortwoertlich im Teaser stehen.
 */
function buildExcerpt(j: JoomlaArticle): string {
  const metadesc = decodeHtmlEntities(
    String((j as Record<string, unknown>)['metadesc'] ?? '').trim(),
  );
  if (metadesc) {
    return metadesc;
  }
  const stripped = stripHtml(j.text ?? '');
  const decoded = decodeHtmlEntities(stripped);
  // Whitespace-Collapse muss NACH dem Decode nochmal laufen, weil
  // dekodiertes &nbsp; zu einem (oder mehreren) Leerzeichen wird.
  return truncate(decoded.replace(/\s+/g, ' ').trim(), 160);
}
